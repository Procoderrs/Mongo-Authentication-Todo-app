...
import toast from 'react-hot-toast';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faAdd, faTrash, faEdit, faCheck } from '@fortawesome/free-solid-svg-icons';
import { Input } from './ui/input';
import { Button } from './ui/button';
import Profile from './Profile';
...
const Todos = () => {
  ...
  const handleAddOrUpdateTodo = async (e) => {
    ...
    if (editingId) {
      // Update existing todo
      try {
        const response = await fetch(`${BACKEND_URL}/api/todos/${editingId}`, {
          method: 'PUT',
  ...
  const handleCompleteTodo = async (id, isCompleted) => {
    try {
      ...
      toast.error('Something went wrong while updating todo status');
    }
  };
          ...
          <div className="mx-auto w-[800px] px-24 py-12 max-w-[820px] flex flex-col gap-6 bg-white rounded-lg">
            ...
            <form onSubmit={handleAddOrUpdateTodo} className="flex gap-10 justify-center items-center">
              <Input
                ...
                onChange={(e) => setCurrentTodo(e.target.value)}
                required
                className="leading-6 py-7 h-12 text-left text-base shadow-md text-black"
              />
