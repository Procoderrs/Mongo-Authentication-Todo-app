...
const Todos = () => {
  ...
  useEffect(() => {
    async function fetchTodos() {
      try {
        const response = await axios.get(`${BACKEND_URL}/api/todos`, { withCredentials: true });
  ...
  async function handleAddOrUpdateTodo(e) {
    ...
    if (editingId) {
      // Optimistic update for updating existing todo
      const updatedTodo = { title: currentTodo, _id: editingId, isCompleted: false };
      setTodos(updatedTodos.map((todo) => (todo._id === editingId ? updatedTodo : todo)));

      try {
        await axios.put(`${BACKEND_URL}/api/todos/${editingId}`, { title: currentTodo }, { withCredentials: true });
        toast.success('Todo updated successfully');
      } catch (error) {
        setTodos(updatedTodos); // Revert the optimistic update
        handleError(error);
      }
      ...
      const newTodo = { title: currentTodo, isCompleted: false };
      setTodos([...todos, { ...newTodo, _id: Date.now().toString() }]); // Adding an optimistic new todo

      try {
        const response = await axios.post(`${BACKEND_URL}/api/todos`, newTodo, { withCredentials: true });
  ...
  async function deleteTodo(id) {
    ...
    setTodos(updatedTodos); // Optimistic delete

    try {
      await axios.delete(`${BACKEND_URL}/api/todos/${id}`, { withCredentials: true });
      toast.success('Todo deleted');
    } catch (error) {
      setTodos(todos); // Revert the optimistic delete
      handleError(error);
    }
  }

  async function handleCompleteTodo(id, isCompleted) {
    const updatedTodos = todos.map((todo) => (todo._id === id ? { ...todo, isCompleted: !isCompleted } : todo));
    setTodos(updatedTodos); // Optimistic complete
    ...
    try {
      ...
      setTodos(todos); // Revert the optimistic complete
      handleError(error);
    }
  }
    ...
    <div className="bg-blue-600 flex items-center justify-center">
      <div className="mx-auto w-[800px] px-24 py-12 max-w-[820px] flex flex-col gap-6 bg-white rounded-lg">
        ...
        {todos?.length ? (
          <div className="py-6 flex flex-col rounded">
            {todos.map((todo) => (
              <div key={todo._id} className="flex h-10 items-center w-full bg-[#c5e0e8] rounded-lg px-5 py-9 mt-2 mb-2">
                <span className={`flex-1 px-3 text-lg ${todo.isCompleted ? 'line-through text-[#63657b]' : ''}`}>
                  {todo.title}
                </span>
                <div className="px-3 flex gap-2">
                  <FontAwesomeIcon
                    className={`bg-[#1d18a3] hover:bg-indigo-700 text-white px-2 py-2 size-4 rounded-lg cursor-pointer ${todo.isCompleted ? 'text-primary' : 'text-slate-300'}`}
                    icon={faCheck}
                    onClick={() => handleCompleteTodo(todo._id, todo.isCompleted)}
                  />
                  <FontAwesomeIcon
