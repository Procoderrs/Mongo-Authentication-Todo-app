...
const LOCAL_URL = "http://localhost:5000";
const VITE_BACKEND_URL = import.meta.env.VITE_BACKEND_URL || "https://mongo-todo-authentication.netlify.app";
const BACKEND_URL = import.meta.env.MODE === "production" ? LOCAL_URL:VITE_BACKEND_URL ;
...
const Todos = () => {
  ...
  async function handleAddOrUpdateTodo(e) {
    ...
    if (editingId) {
      // Update existing todo
      try {
        const response = await axios.put(`${BACKEND_URL}/api/todos/${editingId}`, { title: currentTodo }, { withCredentials: true });
        setTodos(todos.map((todo) => (todo._id === editingId ? { ...todo, title: currentTodo } : todo)));
        setEditingId(null);
        toast.success('Todo updated successfully');
      } catch (error) {
        handleError(error);
      ...
      <div className="bg-blue-600 flex items-center justify-center">
        <div>
          <div className="mx-auto w-[800px] px-24 py-12 max-w-[820px] flex flex-col gap-6 bg-white rounded-lg">
            ...
            {isLoading ? (
              <h1 className="text-2xl py-2 text-center">Loading...</h1>
            ) : todos.length > 0 ? (
              <div className="py-6 flex flex-col rounded">
                {todos.map((todo) => (
